...
namespace TSmatch.Model.Handler
{
    public class ModHandler : Mod
    {
        ...
        public void Hndl(Mod mod)
        {
            // get elmGroups from mod.elements
            var grps = mod.elements.GroupBy(x => x.prf);
            elmGroups.Clear();
            foreach(var gr in grps)
            {
                string mat = gr.First().mat;
                string prf = gr.First().prf;
                ElmGr g = new ElmGr(mod.elements, mat, prf);
                elmGroups.Add(g);
            }
            PrfUpdate();
            // get Pricing for Groups with Rules by Match

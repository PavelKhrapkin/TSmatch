...
namespace TSmatch.ElmAttSet
{
    public class ElmAttSet : IComparable<ElmAttSet> , IEquatable<ElmAttSet>
    {
        ...
        /// <summary>
        /// SumAtt(enum atr, guids) - summarise value of all Elements atr, listed in guids
        /// </summary>
        /// <param name="atr">enum Field reference to be summarised </param>
        /// <param name="guids">List of giuds of the subset of Elements</param>
        /// <returns>total summarised value </returns>
        /// <history>21/6/2016</history>
        internal static double SumAtt(ElmAttSet.sumFields atr, List<string> guids)
        {
            double val = 0.0;
        ...
        /// <history>21/6/2016 moved here from TS_OpenAPI</history>
    ...
    public class Group : IComparable<Group>
    {
        ...
        public Group(string _mat, string _prf, List<string> _guids)
        {
            if (guids.Count == 0) return;
            mat = Lib.ToLat(_mat);
            prf = Lib.ToLat(_prf);
            guids = _guids;
            totalLength = ElmAttSet.SumAtt(ElmAttSet.sumFields.length, guids);
            totalWeight = ElmAttSet.SumAtt(ElmAttSet.sumFields.weight, guids);
            totalVolume = ElmAttSet.SumAtt(ElmAttSet.sumFields.volume, guids);
            totalPrice  = ElmAttSet.SumAtt(ElmAttSet.sumFields.price, guids);
            Groups.Add(this);
        ...
        public static List<Group> setGroups()
        {
            ...
            foreach (var elm in ElmAttSet.Elements)
            {
                ...
                {
                    new Group(curMat, curPrf, curGuids);
                        //////if (lng > 0.0) Groups.Add(new Group(mat, mat_type, prf, lng, wgt, vol, guids));
                    //////mat = elm.mat; mat_type = elm.mat_type; prf = elm.prf; lng = elm.lng; wgt = elm.weight / 1000;
                    //////vol = elm.volume / 1000 / 1000 / 1000;
                    //////guids.Clear(); guids.Add(elm.guid);
                }
            }
            new Group(curMat, curPrf, curGuids);
            return Groups;
